...
def _load_frame(frame_name: str) -> QPixmap:
...
class FrameLoader:
    """Loads and manages QPixmap frames for different mouth states based on a JSON config."""
    ...
    def __init__(self) -> None:
        ...
        self.closed_frame = _load_frame(closed_frame_path)
        self.open_frame = _load_frame(open_frame_path)
        self.intermediate_frames = [_load_frame(p) for p in intermediate_frame_paths]
    ...
    def get_frame_pixmap(self, face_type: FaceType) -> QPixmap:
        """Return the QPixmap for the given face type."""

        if face_type == FaceType.Closed:
            return self.closed_frame
        elif face_type == FaceType.Open:
            return self.open_frame
        elif face_type == FaceType.Intermediate:
